...
(function () {
	...
	L.FLayer.Noaa._currentSpeed = function (dataset, options) {
                var currentoptions = {layers: 'u_velocity_v_velocity', 
	...
	L.FLayer.Noaa._sst = function (dataset, options) {
                var extraoptions = {layers: 'sst', cmap: 'SeaTempArctic_C_BGYR_14colors'}
                options = L.extend(options, extraoptions);
		var layer = new L.FLayer(dataset, options);
		if (layer.options.legendImagePath == null) {
                        layer.options.legendAttribution = '<a href="http://noaa.gov" alt="National Oceanic and Atmospheric Administration">NOAA</a> / HYCOM';
			layer.options.legendImagePath = layer._fcootileurl + '?SERVICE=WMS&REQUEST=GetColorbar&VERSION=1.1.1&LAYERS=sst&STYLES=horizontal,nolabel&FORMAT=image%2Fpng&TRANSPARENT=false&CMAP=SeaTempArctic_C_BGYR_14colors';
		}
		return layer;
	};
}());
