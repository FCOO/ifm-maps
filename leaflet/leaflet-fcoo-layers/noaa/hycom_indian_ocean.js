...
(function () {
	L.FLayer.Noaa.currentSpeed = function (options) {
                ...
                var layer1 = new L.FLayer.Noaa._currentSpeed('NOAA/HYCOM/NOAA_HYCOM_EAST_AFRICA.nc', options);
	...
	L.FLayer.Noaa._currentSpeed = function (dataset, options) {
                var currentoptions = {layers: 'u_velocity_v_velocity', cmap: 'CurrentArctic_kn_GYR_9colors'}
                options = L.extend(options, currentoptions);
		var layer = new L.FLayer(dataset, options);
		if (layer.options.legendImagePath == null) {
                        layer.options.legendAttribution = '<a href="http://noaa.gov" alt="National Oceanic and Atmospheric Administration">NOAA</a> / HYCOM';
			layer.options.legendImagePath = layer._fcootileurl + '?SERVICE=WMS&REQUEST=GetColorbar&VERSION=1.1.1&LAYERS=u_velocity_v_velocity&STYLES=horizontal,nolabel&FORMAT=image%2Fpng&TRANSPARENT=false&CMAP=CurrentArctic_kn_GYR_9colors';
		}
		return layer;
	};
	...
	L.FLayer.Noaa.currentDirection = function (options) {
                options = L.extend(options, {attribution: 'Current forecasts from <a href="http://noaa.gov" alt="National Oceanic and Atmospheric Administration">NOAA</a>'});
                var layer1 = new L.FLayer.Noaa._currentDirection('NOAA/HYCOM/NOAA_HYCOM_EAST_AFRICA.nc', options);
	...
	L.FLayer.Noaa.sss = function (options) {
                ...
                var layer1 = new L.FLayer.Noaa._sss('NOAA/HYCOM/NOAA_HYCOM_EAST_AFRICA.nc', options);
                return new L.FLayerGroup([layer1]);
	};
	L.FLayer.Noaa._sss = function (dataset, options) {
		...
		if (layer.options.legendImagePath == null) {
                        layer.options.legendAttribution = '<a href="http://noaa.gov" alt="National Oceanic and Atmospheric Administration">NOAA</a> / HYCOM';
			layer.options.legendImagePath = layer._fcootileurl + '?SERVICE=WMS&REQUEST=GetColorbar&VERSION=1.1.1&LAYERS=sss&STYLES=horizontal,nolabel&FORMAT=image%2Fpng&TRANSPARENT=false&CMAP=PrSalArctic_psu_GB_16colors';
		}
		return layer;
	};
	...
	L.FLayer.Noaa.sst = function (options) {
                ...
                var layer1 = new L.FLayer.Noaa._sst('NOAA/HYCOM/NOAA_HYCOM_EAST_AFRICA.nc', options);
                return new L.FLayerGroup([layer1]);
	};
	L.FLayer.Noaa._sst = function (dataset, options) {
		...
		if (layer.options.legendImagePath == null) {
                        layer.options.legendAttribution = '<a href="http://noaa.gov" alt="National Oceanic and Atmospheric Administration">NOAA</a> / HYCOM';
			layer.options.legendImagePath = layer._fcootileurl + '?SERVICE=WMS&REQUEST=GetColorbar&VERSION=1.1.1&LAYERS=sst&STYLES=horizontal,nolabel&FORMAT=image%2Fpng&TRANSPARENT=false&CMAP=SeaTemp_C_BGYR_14colors';
		}
		return layer;
	};
}());
